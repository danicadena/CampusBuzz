<?php
	...
	else
	{
		...
		// select admin ID if already an admin
		$getAdminID = $conn->prepare("SELECT Admins_ID FROM Admins WHERE UID = ?");
		$getAdminID->bind_param("i", $newAdmin);
		$getAdminID->execute();
		$getAdminID->bind_result($adminID);
		$getAdminID->fetch();
		$getAdminID->close();
